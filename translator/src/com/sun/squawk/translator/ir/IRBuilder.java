...
public final class IRBuilder {
    ...
    private void parseInstructions(Translator translator, boolean copyThis) {
         ...
         */
        if (Klass.TRACING_ENABLED && Tracer.isTracing("jvmverifier", method.toString())) {
            Tracer.traceln("");
...
/*end[FLOATS]*/
    ...
    private void opc_invokeinit(Method callee) {
        ...
        if (chainedConstructorInvoked) {
            ...
            frame.replaceTypeWithType(Klass.UNINITIALIZED_THIS, method.getDefiningClass());
       ...
       if (thisParameter.isDuped()) {
            ...
            */
           if (chainedConstructorInvoked) {
               Assert.always(thisParameter instanceof LoadLocal && ((LoadLocal)thisParameter).getLocal().getJavacIndex() == 0, "translator can't handle this!");
            }
           ...
           while (true) {
                ...
                if (stackSize != 0 && frame.getStackAt(stackSize - 1) == thisParameter) {
                    // Normal case
                    frame.pop(thisParameter.getType());
                } else {
                    // Handles TCK test: javasoft.sqe.tests.vm.cldc.verifier.indevice005.indevice00501m1_1.indevice00501m1_1_wrapper
                    extraCopies--;
                }
