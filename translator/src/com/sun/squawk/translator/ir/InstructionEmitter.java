...
/*if[TRACING_ENABLED]*/
        this.trace        = Klass.TRACING_ENABLED && Tracer.isTracing("emitter", method.toString());
/*end[TRACING_ENABLED]*/

        /*
         * Work out if the method is in a system class. A system class is one that is romized and
         * is in a java.*, javax.*, or com.sun.* package, but is not a part of the translator.
         * System classes are not preemptable, but application classes must call Thread.yield()
         * every so often.
         */
        if (VM.isHosted()) {
            this.isAppClass = isAppClassHosted(classFile);
        }
    }
     ...
     *
     * @param classFile the class file for the method being converted
     * @return  whether the method is in an application class
     */
    private static boolean isAppClassHosted(ClassFile classFile) throws HostedPragma {
