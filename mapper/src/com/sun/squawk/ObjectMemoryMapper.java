...
public class ObjectMemoryMapper {
    ...
    static class Histogram {
         ...
         */
        private void updateFor(Klass klass, int size) {
            HistogramEntry entry = (HistogramEntry)histogram.get(klass);
            if (entry == null) {
         ...
         */
        private void dump(PrintStream out, String prefix, boolean ofCounts) {
...
/*end[TYPEMAP]*/
     ...
     */
    private Vector fieldDefs = new Vector();
    ...
    private final boolean parseArgs(String[] args) throws IOException {
        ...
        for (; argc != args.length; ++argc) {
            ...
            if (arg.startsWith("-all")) {
                ...
                highlightClasses.addElement(classname);
    ...
    void printOwnedHistogram(Hashtable ownerHistograms, OwnerStats owner, String prefix) {
        Histogram histogram = (Histogram)ownerHistograms.get(owner.name);
    ...
    void printOwnedData(OwnerStats theOwner) {
        ...
        Enumeration objects = ownerTable.keys();
        Vector ownedObjectBuffer = new Vector();
         while (objects.hasMoreElements()) {
                Object object = objects.nextElement();
                OwnerStats owner = (OwnerStats)ownerTable.get(object);
                if (owner == theOwner) {
    ...
    private final void run() {
        try {
            while (!objectMemories.isEmpty()) {
                ...
                if (fieldDefs != null) {
                    for (Enumeration e = fieldDefs.elements(); e.hasMoreElements(); ) {
                ...
                Hashtable ownerHistograms = makeOwnerHistograms();
                ...
                Vector remainingHighlightClasses = new Vector();
    ...
    static class ObjectTable {
        ...
        private static class Stat {
            ...
            private Stat(Address block, int size) {
                this.block = block;
                this.size = size;
            }
        }
        
        void recordObject(Address block, Object oop, int size) {
            objectTable.put(oop, new Stat(block, size));
        }
        
        int getObjectSize(Object oop) {
            Stat stat = (Stat)objectTable.get(oop);
            return stat.size;
        }
        
        Address getObjectBlock(Object oop) {
            Stat stat = (Stat)objectTable.get(oop);
            return stat.block;
        }
    }
     ...
     */
    private Vector highlightClasses = new Vector();
    ...
    private void claimObjectGraph(OwnerStats owner, Object object, boolean countFirstKlass) {
        if (object != Address.zero() && currentObjectMemory.containsAddress(Address.fromObject(object))) {
            ...
            if (cid == CID.KLASS && !countFirstKlass) {
                ...
                OwnerStats oldOwner = (OwnerStats)ownerTable.get(object);
     ...
     */
    private Hashtable makeOwnerHistograms() {
        Hashtable allHistograms = new Hashtable();
        ...
        for (int i = 0; i < highlightClasses.size(); i++) {
            String classname = (String)highlightClasses.elementAt(i);
     ...
     */
    static class ClassOwnerStats extends OwnerStats {
        
        /**
         * Table of name -> ClassOwnerStats.
         */
        static Hashtable allKlassStats;
         ...
         */
        static ClassOwnerStats getKlassStats(String name) {
            return (ClassOwnerStats)allKlassStats.get(name);
        }
        
        static void reset() {
            allKlassStats = new Hashtable();
        }
    ...
    private void registerKlass(Klass klass, Address klassAddr) {
        ...
        if (suiteKlass == null && fullName.equals(suiteKlassName)) {
            suiteKlass = klass;
        }
    ...
    private void printClassTable() {
        OwnerStats[] stats  = tableToArray(ClassOwnerStats.allKlassStats);
     ...
     */
    private OwnerStats[] tableToArray(Hashtable t) {
        OwnerStats[] stats  = new OwnerStats[t.size()];
        Enumeration e = t.elements();
        int i = 0;
        while (e.hasMoreElements()) {
            stats[i++] = (OwnerStats)e.nextElement();
        }
        return stats;
    }
    ...
    private void doPackageAccounting(String name, int size) {
        ...
        while ((last = name.lastIndexOf('.')) != -1) {
            ...
            OwnerStats oldVal = (OwnerStats)packageTable.get(name);
            if (oldVal == null) {
    ...
    void printPackageTable() {
        Enumeration e = ClassOwnerStats.allKlassStats.elements();
        while (e.hasMoreElements()) {
            OwnerStats stat = (OwnerStats)e.nextElement();
...
/*end[TYPEMAP]*/
    ...
    private void registerSuite(Address oop) {
        int size = (suiteKlass.getInstanceSize() * HDR.BYTES_PER_WORD) + HDR.basicHeaderSize;
        ...
        if (!klass.isArray() && methods != Address.zero()) {
            ...
            if (cfLength != length) {
                ...
                for (int i = 0; i != length; ++i) {
                    ...
                    this.methodMap.put(methodBody, "?version-skew, unknown?");
                }
            } else {
