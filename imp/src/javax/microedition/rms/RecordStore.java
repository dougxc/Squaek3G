...
public class RecordStore
{
    ...
    /** RandomAccessFile where this record store is stored */
     ...
     * attributes are read from the RandomAccessFile when the RecordStore
    ...
    boolean exists = RandomAccessFile.exists(recordStoreName);
    ...
    // Create a RandomAccessFile for storing the record store.
    try {
        randomAccessFile = new RandomAccessFile(recordStoreName);
        /*
         * At this point we've opened the RandomAccessFile.  If we
         * created a new record store, initialize the db attributes.
         */
        if (create && !exists) {
        // Initialize record store attributes
        randomAccessFile.seek(RS_SIGNATURE);
        ...
        randomAccessFile.write(DB_INIT);
        } else {
        /*
    ...
    synchronized (dbCacheLock) {
         ...
         */
        int space = RandomAccessFile.spaceAvailable();
    ...
    public static String[] listRecordStores()
    {
        ...
        synchronized (dbCacheLock) {
      String[] returnstrings = RandomAccessFile.listRecordStores();
    ...
    synchronized (dbCacheLock) {
        ...
        if (RandomAccessFile.exists(recordStoreName)) {
            boolean success = RandomAccessFile.deleteFile(recordStoreName);
        if (!success) {
    ...
    synchronized (rsLock) {
        ...
        if (db.opencount <= 0) {  // free stuff - final close
            ...
            if (dbFirstFreeBlockOffset != 0) {
                ...
                randomAccessFile.truncate(dbDataEnd);
            }
            randomAccessFile.close();
            } catch (java.io.IOException ioe) {
    ...
    {
    checkOpen();
    int rv = RandomAccessFile.spaceAvailable() -
        DB_BLOCK_SIZE - DB_RECORD_HEADER_LENGTH;
    return (rv < 0) ? 0 : rv;
    }
    ...
    if (foundBlock == false) {
        ...
        if (RandomAccessFile.spaceAvailable() < allocSize) {
        throw new RecordStoreFullException();
        }
    ...
    try {
        ...
        randomAccessFile.seek(SIGNATURE_LENGTH); // skip RS header 8 bytes
        int numbytes = DB_INIT.length - SIGNATURE_LENGTH;
        randomAccessFile.write(dbState, SIGNATURE_LENGTH, numbytes);
    } catch (java.io.IOException ioe) {
    ...
    int[] getRecordIDs()
    {
    if (randomAccessFile == null) // lower overhead than checkOpen()
        return null;
    ...
    while (offset < dbDataEnd) {
        ...
        if (target == 0) {
            ...
            while (bytesLeft > 0) {
                ...
                randomAccessFile.read(chunkBuffer, 0, numToMove);
                randomAccessFile.seek(target);
                randomAccessFile.write(chunkBuffer, 0, numToMove);
                offset += numToMove;
                target += numToMove;
                bytesLeft -= numToMove;
            }
     ...
     * read from a RandomAccessFile at offset <code> offset </code>.
     *
     * @param _offset seek offset in RandomAccessFile of desired
     *        RecordHeader.
     *
     ...
     *
     * @param _offset offset in RandomAccessFile where this
    ...
    void load(int _offset) throws java.io.IOException {
        ...
        randomAccessFile.seek(offset);
        randomAccessFile.read(recHeadBuf, 0, DB_RECORD_HEADER_LENGTH);
     ...
     * a RandomAccessFile at <code>offset</code>.
    ...
    void store() throws java.io.IOException {
        ...
        // write record header;
        randomAccessFile.seek(offset);
        randomAccessFile.write(recHeadBuf, 0, DB_RECORD_HEADER_LENGTH);
    }

    /**
     * Reads data associated with this record from storage (a
     * RandomAccessFile) at <code>offset</code>.
     *
    ...
    int read(byte[] buf, int _offset) throws java.io.IOException {
        randomAccessFile.seek(offset + DATA_OFFSET);
        return randomAccessFile.read(buf, _offset, dataLenOrNextFree);
    }
    ...
    void write(byte[] buf, int _offset) throws java.io.IOException {
        randomAccessFile.seek(offset + DATA_OFFSET);
        randomAccessFile.write(buf, _offset, dataLenOrNextFree);
    }
    }
