...
public class TestC17 implements Types {
    ...
    public static void main(String[] args) {
        ...
        Local resultCount = c.local(INT);
         c.result(INT);
             c.begin();
                ...
                c.literal(2).literal(gt2).call(2, INT);
                printResult(c, "compare (5 > 2) = \u0000", " %d.  Expecting: 1.\n\u0000", 1);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(10).literal(gt2).call(2, INT);
                printResult(c, "compare (5 > 10) = \u0000", " %d.  Expecting: 0.\n\u0000", 0);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(2).literal(gt3).call(2, INT);
                printResult(c, "compare (2 > 5) = \u0000", " %d.  Expecting: 0.\n\u0000", 0);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(10).literal(gt3).call(2, INT);
        ...
        System.out.println("Greater than tests failed = " + res);
        System.exit(res);
    }
