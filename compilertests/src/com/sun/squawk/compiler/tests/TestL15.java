...
public class TestL15 implements Types {
    ...
    public static void main(String[] args) {
        ...
        Local resultCount = c.local(INT);
         c.result(INT);
             c.begin();
                ...
                c.literal(2L).literal(ne2).call(2, INT);
                printResult(c, "compare (5 != 2) = \u0000", " %d.  Expecting: 1.\n\u0000", 1);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(10L).literal(ne2).call(2, INT);
                printResult(c, "compare (5 != 10) = \u0000", " %d.  Expecting: 1.\n\u0000", 1);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(2L).literal(ne3).call(2, INT);
                printResult(c, "compare (2 != 5) = \u0000", " %d.  Expecting: 1.\n\u0000", 1);
                c.load(resultCount).swap().sub().store(resultCount);

                c.literal(10L).literal(ne3).call(2, INT);
        ...
        System.out.println("Long not equal to tests failed = " + res);
        System.exit(res);
    }
