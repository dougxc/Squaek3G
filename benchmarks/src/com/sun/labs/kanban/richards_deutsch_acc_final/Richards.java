...
package com.sun.labs.kanban.richards_deutsch_acc_final;
...
final class DeviceTaskRec extends TaskRec {
...
final class IdleTaskRec extends TaskRec {
...
final class HandlerTaskRec extends TaskRec {
...
final class WorkerTaskRec extends TaskRec {
...
class TaskState {
  ...
  final TaskState PacketPending() {
    packetPending = true;
    taskWaiting = taskHolding = false;
    return this;
  }
  final TaskState Waiting() {
    packetPending = taskHolding = false;
    taskWaiting = true;
    return this;
  }
  final TaskState Running() {
    packetPending = taskWaiting = taskHolding = false;
    return this;
  }
  final TaskState WaitingWithPacket() {
    packetPending = taskWaiting = true; taskHolding = false;
    return this;
  }

  /* accessing */
  final boolean IsPacketPending() { return packetPending; }
  final boolean IsTaskWaiting()   { return taskWaiting; }
  final boolean IsTaskHolding()   { return taskHolding; }

  final void SetTaskHolding(boolean state) { taskHolding = state; }
  final void SetTaskWaiting(boolean state) { taskWaiting = state; }
  final void SetPacketPending(boolean state) { packetPending = state; }

  /* testing */ 
  final boolean IsTaskHoldingOrWaiting() {
    return IsTaskHolding() || !IsPacketPending() && IsTaskWaiting();
  }
  final boolean IsWaitingWithPacket() {
...
abstract class Task extends TaskState {
  ...
  final Task      Link() { return link; }
  final int      Ident() { return id; }
  final int   Priority() { return pri; }
  final Packet   Input() { return wkq; }
  final TaskRec Handle() { return handle; }

  final void     SetLink(Task x)    { link = x; }
  final void    SetIdent(int x)     { id = x; }
  final void SetPriority(int x)     { pri = x; }
  final void    SetInput(Packet x)  { wkq = x; }
  final void   SetHandle(TaskRec x) { handle = x; }

  static final int TaskTabSize = 10;
  private static Task[] taskTab = new Task[TaskTabSize];
  final static Task    TaskTab(int i)         { return taskTab[i]; }
  final static void SetTaskTab(int i, Task t) { taskTab[i] = t; }
  ...
  final static int  get_holdCount()      { return holdCount; }
  final static void set_holdCount(int i) { holdCount = i; }

  final static int  get_queuePacketCount()      { return qpktCount; }
  final static void set_queuePacketCount(int i) { qpktCount = i; }
  ...
  final protected Task waitTask() {
    SetTaskWaiting(true);
    return this;
  }

  final protected Task hold() {
    set_holdCount(get_holdCount() + 1);
    SetTaskHolding(true);
    return Link();
  }

  final protected Task release(int i) {
    Task t = findtcb(i);
    t.SetTaskHolding(false);
    return t.Priority() > Priority() ? t : this;
  }

  final protected Task qpkt(Packet pkt) {
  ...
  final static Task findtcb(int id) {
  ...
  final static void trace(char a) {
    if (--layout <= 0) {
      System.out.println();
      layout = 50;
    }
...
final class WorkTask extends Task 
{
...
final public class Richards {
  ...
  public void inst_main(String[] args) { 
    System.out.println("Richards benchmark (deutsch_acc_final) starting...");
