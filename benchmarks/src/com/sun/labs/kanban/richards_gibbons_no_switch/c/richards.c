...
/*  Switch-less version with bools for state, M. Wolczko,
...
createtask(id,pri,wkq,pp,tw,th,fn,v1,v2)
    int id,pri,pp,tw,th,v1,v2;
    struct packet *wkq;
    struct task *(*fn)();
{
    struct task *t;
...
struct task *runtask(struct task *current)
{
    struct packet *pkt;
    struct task *newtcb;
    if (current->t_pp && current->t_tw && !current->t_th) {
        pkt = current->t_wkq;
	current->t_wkq = pkt->p_link;
	if (current->t_wkq == 0)
	    current->t_pp = FALSE;
	else
	    current->t_pp = TRUE;
	current->t_tw = current->t_th = FALSE;
    } else
        pkt = 0;
    v1.i = current->t_v1;
    v2.i = current->t_v2;
    newtcb = (current->t_fn)(pkt);
    current->t_v1 = v1.i;
    current->t_v2 = v2.i;
    return newtcb;
}
...
schedule()
{
    ...
    {
    	numIter++;
/*     if (tracing==TRUE) printf("tcb = %d\n", tcb); */

	if (tcb->t_th || !tcb->t_pp && tcb->t_tw)
	    tcb = tcb->t_link;
	else {
	    taskid = tcb->t_id;
...
main()
{
  ...
  for (i = 0;  i < ITER;  ++i) {
    ...
    createtask(I_IDLE, 0, wkq, FALSE, FALSE, FALSE, idlefn, 1, 10000);
printf("idle: %d\n", clock());
    wkq = pkt(0, 0, K_WORK);
    wkq = pkt(wkq, 0, K_WORK);

    createtask(I_WORK, 1000, wkq, TRUE, TRUE, FALSE, workfn, I_HANDLERA, 0);
printf("work: %d\n", clock());
    ...
    createtask(I_HANDLERA, 2000, wkq, TRUE, TRUE, FALSE, handlerfn, 0, 0);
printf("handler a: %d\n", clock());
    ...
    createtask(I_HANDLERB, 3000, wkq, TRUE, TRUE, FALSE, handlerfn, 0, 0);
printf("handler b: %d\n", clock());

    wkq = 0;
    createtask(I_DEVA, 4000, wkq, FALSE, TRUE, FALSE, devfn, 0, 0);
    createtask(I_DEVB, 5000, wkq, FALSE, TRUE, FALSE, devfn, 0, 0);
printf("devices: %d\n", clock());
    tcb = tasklist;
