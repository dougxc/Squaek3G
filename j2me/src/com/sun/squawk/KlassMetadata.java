...
public final class KlassMetadata {
     ...
     */
    private KlassMetadata(byte[] symbols, Klass[] classTable, KlassMetadata original) {
    ...
    static KlassMetadata[] strip(Suite suite, KlassMetadata[] metadatas, int type) {
        ...
        for (int i = 0; i != metadatas.length; ++i) {
            ...
            if (metadata != null) {
                switch (type) {
                    case Suite.DEBUG:
                        newMetadatas[i] = new KlassMetadata(metadata.symbols, metadata.classTable, metadata);
                        break;
                    case Suite.APPLICATION:
                        newMetadatas[i] = new KlassMetadata(null, null, metadata);
                        break;
                    case Suite.LIBRARY:
    ...
    private KlassMetadata strip(int type) {
        ...
        byte[] symbols = getSymbolParser().strip(definedClass, type, types);
        Klass[] classTable = new Klass[types.size()];
        types.copyInto(classTable);
        return new KlassMetadata(symbols, classTable, this);
    }
