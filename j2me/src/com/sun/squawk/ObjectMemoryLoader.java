...
/*if[FLASH_MEMORY]*/
        // The bootstrap suite has the uri memory://bootstrap, thus it is not loaded and not verified. (as intented)            
        if (uri.startsWith("flash:")) {
            ...
            try {
            	// uri will be the URL to the flash:address
                suiteIn = Connector.openInputStream(uri);
                try {
                	if (!SignatureVerifier.isVerifiedSuite(uri) && !noPublicKeyInstalled) {
                		if (VM.isVerbose())                			
...
//if(!VM.isHosted()) GC.traverseMemory(relocatedBuffer, relocatedBuffer.add(size), true, true);
        if (Klass.TRACING_ENABLED && !VM.isHosted()) {
            boolean trace = Tracer.isTracing("oms");
            GC.traverseMemory(relocatedBuffer, relocatedBuffer.add(size), trace, true);
        }
    ...
    private static String convertURIHosted(String uri) throws HostedPragma {
        if (uri.equals(ObjectMemory.BOOTSTRAP_URI)) {
            String url = System.getProperty("bootstrap.suite.url");
            Assert.always(url != null);
            return url;
        }
        else
            return uri;
    ...
    private ObjectMemory loadParent(int hash, String uri) {
        ...
        if (parent == null) {
            try {
                ...
                parent = load(dis, uri, loadIntoReadOnlyMemory).objectMemory;
                dis.close();
            } catch (IOException e) {
                throw new Error("IO error loading parent object memory from '" + uri + "': " + e);
            }
        }
        ...
        if (parent.getHash() != hash) {
        	String helpText = "";
/*if[FLASH_MEMORY]*/
        	if ("flash://lib".equals(parent.getURI())) {
