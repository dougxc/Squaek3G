 ...
 * to this Mailbox. A Mailbox can accept messages from any number of clients, and these messages are queued in the order received.
 *
 ...
 * to the same Mailbox. This allows multiple logical connections to be managed by the same message queue.
 *
 ...
 * or when the Mailbox is closed, which invalidates the MailboxAddresses that refer to it.
 *
 ...
 * A MailboxHandler may create a new MailBox and new Mailbox address, in order to support private Mailboxes for different clients (see {@link ServerChannel}).
 *
...
public final class Mailbox {
    ...
    public synchronized void close() {
        if (Isolate.currentIsolate() != owner) {
            throw new IllegalStateException("Attempting to close " + this + " by " + Isolate.currentIsolate());
        }
