...
public class Board {
  ...
  public void computerMove( int side )
  {
    ...
    if (choice.score <= alpha && !timeout_flag ) {
       ...
       Main.println( "failed low: " +  choice.score);
       ...
       Main.println( "failed high: " +  choice.score);
    ...
    beta  = choice.score+50;
      ...
      if (new_move.score <= alpha && !timeout_flag ) {
         ...
         Main.println( "failed low: " +  new_move.score);
       ...
       beta  = WIN_SCORE;
         ...
         Main.println( "failed high: " +  new_move.score);
      ...
      // changed June 17, 1999 by RL
        ...
        // Main.println( "Move: " + depth + " " + new_move.score +
        //                     " (" + new_move.from_row() + ", "
        //                     + new_move.from_col() + " ) -> ( " +
        //                    new_move.to_row() + ", " + new_move.to_col() + " )" );
        Main.print( "Move: " + depth + " " + new_move.score + " " );

        if (pondering && !puzzling) Main.print("(" + new Move(hint).string() + ")" );

        Main.println(pv.display());

        long t = System.currentTimeMillis() - search_start_time;

        Main.println( "Nodes: " + search_counter + "  " +
    ...
    if (chk_hash != hashKey)
      ...
      Main.println( "Hash: " + chk_hash + " " + hashKey);
  ...
  private Move selectMove(int ply, int depth, int side, int alpha, int beta, boolean quiesce )
  {
    ...
    // debug (RL)
    // Main.println("ply: "+ ply + " " + alpha + " " + beta);
...
//        Main.print(" "+m3.score+"-");
    ...
    int ext=0;
      ...
      if (ply == 1 && !pondering) {
         ...
         //Main.println("random: " + random_score);
    ...
    if (ply == 1 && !timeout_flag)
      ...
      if( -opponent_move.score > best_move.score && !timeout_flag){
        ...
        //   Main.println( "PV: " + best_move.score + " " + pv.display());
    ...
    // debug
      ...
      //   Main.println( "debug: " + ply + " " +

      //                   best_move.score + " " + alpha + " " + beta);

      // if (ply==1)

      //   Main.println( "debug: " + m.string() + " " + -opponent_move.score);
